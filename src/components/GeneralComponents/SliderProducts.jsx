...
export default function SliderProducts(onlyFor) {
    ...
    return (
        <Container key={[Navigation]} sx={{marginTop: '10px'}}>
            <Swiper key={[Navigation]} navigation={true} modules={[Navigation]} slidesPerView={3}
            spaceBetween={30} slidesPerGroup={3} loop={true} loopFillGroupWithBlank={true} className="mySwiper SliderProductsConfirm">
                {Products.map(item => {
                    return (
                        <SwiperSlide tag="div" className="productCard" key={item.id}>
                            <Box id="btnOpenPopUp" className="openProductDetails" key={item.id}>
                                <Link to={'/Product/' + item.id} target='_blank'>
                                    ...
                                    <Box className="des">
                                        <Typography variant="h5" color="#222" sx={{fontSize: '18px', fontWeight: '400', padding: '5px 0', height: '58.02px'}}>
                                            {item.name}
                                        </Typography>
